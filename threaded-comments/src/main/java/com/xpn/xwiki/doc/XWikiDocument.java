...
public class XWikiDocument {
    ...
    public int getObjectDepth(String classname, String property, int number) {
        try {
            BaseObject object = getObject(classname,number);
            int parent = ((Integer) ((BaseProperty) object.getField(property)).getValue()).intValue();
            if (parent >= 0) 
                return getObjectDepth(classname,property,parent)+1;
            return 0;
        } catch(Exception e) {
            return 0;
        }
    ...
    /**
     * This method can be used to create a tree structure from a list of attached
     * objects of a given class and having a property linking to each object's
     * parent number. The method returns the root node of a tree built as
     * follow: the list of objects being of class "classname" is retrieved, then
     * a root node is created, its children are the nodes whose "property" field values "-1"
     * and recursively the value of the "property" field is used as a pointer
     * to the partent node. The method is used to display threaded comments.
     * 
     * @param classname
     * @param property
     * @return
     */
    public TreeNode getObjectsTree(String classname, String property, XWikiContext context) {
        try {            
            Vector baseobjects = (Vector) getObjects(classname);
            if (Util.isEmpty(baseobjects))
                return null;
            
            Vector objects = new Vector();
            for (int i=0;i<baseobjects.size();i++) {
                BaseObject bobj = (BaseObject) baseobjects.get(i);
                if (bobj!=null) {
                  objects.add(new com.xpn.xwiki.api.Object(bobj, context));
                }
            }
            
            TreeNode tn = new TreeNode();
            tn.insert(objects, property);
            
            return tn;

        } catch(Exception e) {
            e.printStackTrace();
            log.error("getObjectsTree", e);
            return null; 
        }
    }
    ...
    //:SL: added
    public void removeObject(String classname, BaseObject object) {
        Vector objects = getObjects(classname);
        if (objects != null && objects.indexOf(object) >= 0)
    ...
    */
    ...
    public Document toXMLDocument(boolean bWithObjects, boolean bWithRendering,
        ...
        el = new DOMElement("rights");
        el.addText(getRights());
        docel.add(el);
    ...
    public void fromXML(String xml, boolean withArchive) throws XWikiException {
        ...
        setRights(getElement(docel,"rights"));
