...
import com.xpn.xwiki.plugin.workspacesmanager.stories.DefaultStoryBuilderImpl;
import com.xpn.xwiki.plugin.workspacesmanager.stories.PhotosStoryBuilderImpl;
import com.xpn.xwiki.plugin.workspacesmanager.stories.Story;
import com.xpn.xwiki.plugin.workspacesmanager.stories.StoryBuilder;
...
public class WorkspacesManager extends SpaceManagerImpl
{
        ...
        throws WorkspacesManagerException
    {
        try {
            this.addRightToGroup(spaceName, getRoleGroupName(spaceName,
                WorkspacesManagerExtension.WORKSPACE_ROLE_READER_CODE), "view,comment,edit",
                true, true, context);
        } catch (XWikiException e) {
            throw new WorkspacesManagerException(e);
        }
    ...
    /**
     * Returns, for a workspace, the list of {@link stories.Story} associated with the last modified
     * documents. Stories are built by a {@link stories.StoryBuilder} chosen for each story
     * according to the application the modified document belong to.
     * 
     * @param spaceName the XWiki name of the workspace to return the stories for
     * @param startAt the offset to start retrieving modified documents at in the workspace
     * @param howMany the number of stories to retrieve
     * @throws WorkspacesManagerException
     */
    public List getStoriesForSpace(String spaceName, int startAt, int howMany,
        XWikiContext context) throws WorkspacesManagerException
    {
        String allWebs =
            "select distinct doc.web from XWikiDocument as doc, XWikiDocument as sp"
                ...
                + spaceName + "'";
        String hql =
            "where (doc.web in (" + allWebs
                + ") and doc.name<>'WebPreferences' and doc.name<>'WebHome') or (doc.web='"
                + spaceName + "' and doc.name='WebHome') order by doc.contentUpdateDate desc";
        try {
            StoryBuilder sb = new DefaultStoryBuilderImpl();
            StoryBuilder psb = new PhotosStoryBuilderImpl();
            List docs =
                context.getWiki().getStore().searchDocumentsNames(hql, howMany, startAt, context);
            List result = new ArrayList();
            for (Iterator it = docs.iterator(); it.hasNext();) {
                String docName = (String) it.next();
                try {
                    Story st;
                    // I don't like this test that makes the workspace manager aware of
                    // the workspaces applications.
                    // This should disappear when using the activity stream plugin
                    // to generate the stories for workspaces.
                    if (getApplicationName(docName, context).equals("Photos"))
                        st = psb.createStory(spaceName, docName, context);
                    else
                        st = sb.createStory(spaceName, docName, context);
                    result.add(st);
    ...
    /**
     * @return the wiki name of the space the passed web belongs to. (for example : Space_Spacename for
     * the web Space_Spacename_Wiki.
     */
