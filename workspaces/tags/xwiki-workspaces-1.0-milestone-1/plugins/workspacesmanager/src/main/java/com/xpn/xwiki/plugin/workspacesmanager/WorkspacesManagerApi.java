...
public class WorkspacesManagerApi extends SpaceManagerPluginApi
{
    ...
    {
        try {
            // look over spaces in which the context user belongs to the member group
     ...
     * @return the top level space (or web) for the workspace's given web, corresponding to the level the passed
     *         web inherits its rights from, through the 'parent' property of its WebPreference. As an example, the web
     ...
     * @param allowDowngrade allow or not, to downgrade the role of the user within the workspace, if
     * this one already is member with a higher role (writer or admin)
     */
    ...
    public List getStories(String spaceName, int howMany, int startAt)
    {
        try {
            return getWorkspacesManager()
                .getStoriesForSpace(spaceName, startAt, howMany, context);
        } catch (WorkspacesManagerException e) {
            context.put("haserror", "1");
            context.put("lasterror", e.getMessage());
            return Collections.EMPTY_LIST;
