...
DatePicker.prototype	= {
 ...
 load		: function () {
  /* append to body */
  var overlayDiv = $('TOverlay');
  //document.getElementsByTagName("body").item(0);
  if ( overlayDiv )
   overlayDiv.appendChild( this._div );
  /* position the datepicker relatively to element */
  var a_lt = Position.positionedOffset($(this._relative));
  $(this._id_datepicker).style.left	= Number(a_lt[0])+'px';
  $(this._id_datepicker).style.top	= Number(a_lt[1]+30)+'px';
  ...
  */
 click		: function () {
  if ( !Element.visible(this._id_datepicker) ) {
   this._initCurrentDate();
  ...
  //new Effect.toggle(, 'appear');
  Effect.Appear(this._id_datepicker, { duration: 0.2 });
 },
 /**
 ...
 _buildCalendar		: function () {
  ...
  for ( var i = 0; i < 6; i++ ) {
   ...
   for ( var j = 0; j < 7; j++ ) {
    ...
    td.onclick	= function () { 
     $(_self._relative).setAttribute('value', 
       String($(this).id
 ...
 _setLocaleHdr	: function () {
  ...
  /* header */
  Element.update(this._id_datepicker_hdr,'&nbsp;&nbsp;&nbsp;'+this.getMonthLocale(this._current_mon)+'&nbsp;'+this._current_year+'&nbsp;&nbsp;&nbsp;');
 }
};
