    ...
    public void postDeleteSpace(String spaceName, boolean deleteData,
		...
		if (deleteData) {
			...
			String hql = ", XWikiDocument as sp"
					+ ", BaseObject as prefs, StringProperty as parent"
					...
					+ spaceName + "'";
			try {
				List<XWikiDocument> allSpaceDocs = context.getWiki().getStore()
						.searchDocuments(hql, context);
				for (XWikiDocument toBeDeleted : allSpaceDocs) {
					// delete document, skipping the trash bin.
					context.getWiki().deleteDocument(toBeDeleted, false,
							context);
				}
			} catch (XWikiException e) {
				// silently fail but log the error
				LOG.error(
						"Failed to delete workspace data after deleting workspace ["
								+ spaceName + "]", e);
			}
		}
	}
