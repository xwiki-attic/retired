...
public class WorkspacesManagerApi extends SpaceManagerPluginApi
{
    ...
    /**
     * @return the list of space Names in which the context user is a member and has a specific role
     *         (i.e. admin, reader or writer).
     */
    public Collection<String> getMyMembershipsSpacesNames()
    {
        try {
            return getWorkspacesManager().getMembershipSpacesNamesForUser(context.getUser(), 0,
                0, context);
        } catch (SpaceManagerException e) {
            context.put("haserror", "1");
            context.put("lasterror", e.getMessage());
