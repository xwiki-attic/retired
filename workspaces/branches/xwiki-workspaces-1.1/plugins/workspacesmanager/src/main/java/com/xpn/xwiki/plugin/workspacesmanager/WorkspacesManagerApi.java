...
public class WorkspacesManagerApi extends SpaceManagerPluginApi
{
     ...
     * @return the numbers of members of the given workspace, that is the total number of XWiki.XWikiGroups objects in
     *         the space members group document of the workspace.
     */
    ...
    /**
     * Returns, ordered alphabetically, a set of all {@link Workspace} the context user can access to, at least with the
     * "view" access level. This includes :
     * <ul>
     * <li>His own personal space if it exists</li>
     * <li>The spaces he has a role in, as a reader, writer or administrator</li>
     * <li>All the open and public spaces, that are viewable by anyone authenticated</li>
     * <li>The organization space if it exists, necessarily open or public</li>
     * </ul>
     */
    public Set<String> getAllMySpacesNames()
    {
        try {
            return getAllSpacesForUser(context.getUser()).keySet();
        } catch (SpaceManagerException e) {
    ...
    /**
     * @return A map of space names as keys and {@Workspace} as values, of all workspaces the user with passed username
     *         can access to This includes :
     *         <ul>
     *         <li>His own personal space if it exists</li>
     *         <li>The spaces he has a role in, as a reader, writer or administrator</li>
     *         <li>All the open and public spaces, that are viewable by anyone authenticated</li>
     *         <li>The organization space if it exists, necessarily open or public</li>
     *         </ul>
     *         The space name key set is ordered alphabetically.
     * @throws SpaceManagerException
     */
    private Map<String, Space> getAllSpacesForUser(String username) throws SpaceManagerException
    {
        TreeMap<String, Space> spaces = new TreeMap<String, Space>();
        Space userSpace = getSpaceForUser(username);
        Space organizationSpace = getOrganizationSpace();
        if (userSpace != null) {
            spaces.put(userSpace.getSpaceName(), userSpace);
        }
        if (organizationSpace != null) {
            spaces.put(organizationSpace.getSpaceName(), organizationSpace);
        }
        for (String openSpaceName : getOpenSpacesNames()) {
            spaces.put(openSpaceName, getSpace(openSpaceName));
        }
        for (String publicSpaceName : getPublicSpacesNames()) {
            spaces.put(publicSpaceName, getSpace(publicSpaceName));
        }
        for (String membershipSpaceName : getWorkspacesManager().getMembershipSpacesNamesForUser(username, 0, 0,
            context)) {
            spaces.put(membershipSpaceName, getSpace(membershipSpaceName));
        }
        return spaces;
    }

    /**
     * @return the list of {@link Workspace} in which the context user is a member and has a specific role (i.e. admin,
     *         reader or writer).
     */
    public Collection<Space> getMyMemberships()
    {
        try {
            return getWorkspacesManager().getMembershipSpaces(context.getUser(), 0, 0, context);
        } catch (SpaceManagerException e) {
            context.put("haserror", "1");
            context.put("lasterror", e.getMessage());
    ...
    /**
     * @return the list of space Names in which the context user is a member and has a specific role (i.e. admin, reader
     *         or writer).
     */
    public Collection<String> getMyMembershipsSpacesNames()
    {
        try {
            return getWorkspacesManager().getMembershipSpacesNamesForUser(context.getUser(), 0, 0, context);
        } catch (SpaceManagerException e) {
            context.put("haserror", "1");
            context.put("lasterror", e.getMessage());
            return Collections.emptyList();
    ...
    /**
     * Lookup the wiki users that are not already members of the asked workspace. This method is intended to be used
     * when offering a UI to add wiki users to the workspace.
     * 
     * @param spaceName the wiki name of the workspace to retrieve the users that does not belong to it
     ...
     * @return the potential members for the workspace, which are the users of the current wiki that does not belong to
     *         the member group of the workspace
     */
    public Collection getPotentialMembers(String spaceName, int howMany, int startAt)
    {
        try {
            return getWorkspacesManager().getPotentialMembersForSpace(spaceName, howMany, startAt, context);
    ...
    /**
     * @param matching the sequence of letters that either the first name or the last name of the potential members to
     *            retrieve must contains.
     * @see #getPotentialMembers(String, int, int)
     * @return the potential members for the workspace, whose first name or last name is matching the passed pattern
     */
    public Collection getPotentialMembers(String spaceName, String matching, int howMany, int startAt)
    {
        try {
            return getWorkspacesManager().getPotentialMembersForSpace(spaceName, matching, howMany, startAt, context);
        } catch (WorkspacesManagerException e) {
            context.put("haserror", "1");
            context.put("lasterror", e.getMessage());
            return Collections.EMPTY_LIST;
    ...
    /**
     * @return the global admins list
     */
    public Collection getGlobalAdmins()
    {
        if (hasProgrammingRights()) {
            try {
                return getWorkspacesManager().getGlobalAdmins(0, 0, context);
            } catch (SpaceManagerException e) {
                context.put("haserror", "1");
                context.put("lasterror", e.getMessage());
                return Collections.EMPTY_LIST;
    ...
    /**
     * @return the power users list
     */
    public Collection getPowerUsers()
    {
        if (hasProgrammingRights()) {
            try {
                return getWorkspacesManager().getPowerUsers(0, 0, context);
            } catch (SpaceManagerException e) {
                context.put("haserror", "1");
                context.put("lasterror", e.getMessage());
                return Collections.EMPTY_LIST;
    ...
    /**
     * @return the organization insiders
     */
    public Collection getInsiders()
    {
        if (hasProgrammingRights()) {
            try {
                return getWorkspacesManager().getInsiders(0, 0, context);
            } catch (SpaceManagerException e) {
                context.put("haserror", "1");
                context.put("lasterror", e.getMessage());
                return Collections.EMPTY_LIST;
    ...
    /**
     * @return the organization outsiders
     */
    public Collection getOutsiders()
    {
        if (hasProgrammingRights()) {
            try {
                return getWorkspacesManager().getOutsiders(0, 0, context);
            } catch (SpaceManagerException e) {
                context.put("haserror", "1");
                context.put("lasterror", e.getMessage());
                return Collections.EMPTY_LIST;
     ...
     * @return if, exists, the unique user workspace for the given user wikiname, null if it does not exists.
     */
     ...
     * @return the top level space (or web) for the workspace's given web, corresponding to the level the passed web
     *         inherits its rights from, through the 'parent' property of its WebPreference. As an example, the web
     *         'Space_SomeSpace_Blog' will have 'Space_SomeSpace' as its workspace root space.
     */
    ...
    /**
     * Add a user as a reader to the space, adding to the reader role group. If not already member, adds it to the
     * member group too.
     * 
     ...
     * @param allowDowngrade allow or not, to downgrade the role of the user within the workspace, if this one already
     *            is member with a higher role (writer or admin)
     */
     ...
     * @param userName
     */
    public void removeGlobalAdmin(String userName)
    {
        if (hasProgrammingRights()) {
            try {
                getWorkspacesManager().removeGlobalAdmin(userName, context);
            } catch (SpaceManagerException e) {
                context.put("haserror", "1");
                context.put("lasterror", e.getMessage());
            }
     ...
     * @param userName
     */
    public void removePowerUser(String userName)
    {
        if (hasProgrammingRights()) {
            try {
                getWorkspacesManager().removePowerUser(userName, context);
            } catch (SpaceManagerException e) {
                context.put("haserror", "1");
                context.put("lasterror", e.getMessage());
            }
     ...
     * @param userName
     */
    public void moveToInsiders(String userName)
    {
        if (hasProgrammingRights()) {
            try {
                getWorkspacesManager().moveToInsiders(userName, context);
            } catch (SpaceManagerException e) {
                context.put("haserror", "1");
                context.put("lasterror", e.getMessage());
            }
     ...
     * @param userName
     */
    public void moveToOutsiders(String userName)
    {
        if (hasProgrammingRights()) {
            try {
                getWorkspacesManager().moveToOutsiders(userName, context);
            } catch (SpaceManagerException e) {
                context.put("haserror", "1");
                context.put("lasterror", e.getMessage());
            }
    ...
    public boolean isGlobalAdmin()
    {
        ...
        try {
            globalAdmins = getWorkspacesManager().getGlobalAdmins(0, 0, context);
            String userName = context.getUser();
            for (Object obj : globalAdmins) {
                if (((String) obj).equals(userName)) {
                    return true;
                }
            }
        } catch (SpaceManagerException e) {
    ...
    public boolean isInsider()
    {
        Collection insiders;
        try {
            insiders = getWorkspacesManager().getInsiders(0, 0, context);
            String userName = context.getUser();
    ...
    public boolean isOutsider()
    {
        ...
        try {
            outsiders = getWorkspacesManager().getOutsiders(0, 0, context);
            String userName = context.getUser();
            for (Object obj : outsiders) {
                if (((String) obj).equals(userName)) {
                    return true;
                }
            }
    ...
    public void addGlobalAdmin(String username)
    {
        if (hasProgrammingRights()) {
            try {
                getWorkspacesManager().addGlobalAdmin(username, context);
            } catch (SpaceManagerException e) {
                context.put("haserror", "1");
                context.put("lasterror", e.getMessage());
            }
    ...
    public void addPowerUser(String username)
    {
        if (hasProgrammingRights()) {
            try {
                getWorkspacesManager().addPowerUser(username, context);
            } catch (SpaceManagerException e) {
                context.put("haserror", "1");
                context.put("lasterror", e.getMessage());
            }
     ...
     */
    public String getApplicationURL(String spaceName, String appName, String docName, String queryString)
    {
        return getApplicationsManagerApi().getApplicationURL(spaceName, appName, docName, queryString);
    }

    /**
     * @deprecated use {@link activities.WorkspacesActivityStreamPluginApi#getEventsForWorkspace} instead
     */
    ...
    public List getStories(String spaceName, int howMany, int startAt)
    {
        WorkspacesActivityStreamPluginApi wasApi =
            (WorkspacesActivityStreamPluginApi) context.getWiki().getPluginApi("xwsactivitystream", context);
    ...
    {
        try {
            return getWorkspacesManager().searchDocuments(spaceName, applicationName, howMany, startAt, context);
        } catch (WorkspacesManagerException e) {
            context.put("haserror", "1");
    ...
    public Collection<String> getPublicSpacesNames()
    {
        try {
            return getWorkspacesManager().getPublicSpacesNames(context);
        } catch (WorkspacesManagerException e) {
            context.put("haserror", "1");
            context.put("lasterror", e.getMessage());
            return Collections.EMPTY_LIST;
    ...
    public Collection<Space> getOpenSpaces()
    {
        try {
    ...
    public Collection<String> getOpenSpacesNames()
    {
        try {
            return getWorkspacesManager().getOpenSpacesNames(context);
        } catch (WorkspacesManagerException e) {
            context.put("haserror", "1");
            context.put("lasterror", e.getMessage());
            return Collections.emptyList();
