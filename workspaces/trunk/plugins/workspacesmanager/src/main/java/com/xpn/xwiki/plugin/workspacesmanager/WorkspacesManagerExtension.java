...
import com.xpn.xwiki.plugin.workspacesmanager.apps.DefaultWorkspaceApplicationManager;
...
public class WorkspacesManagerExtension extends SpaceManagerExtensionImpl
{
     ...
     * {@inheritDoc}
     */
    public void virtualInit(SpaceManager manager, XWikiContext context) throws SpaceManagerException
    {
        try {
            updateWorkspaceSpaceClass(context);
            this.sm = manager;
    ...
    private WorkspaceApplicationManager getXWSApplicationManager()
    {
        if (this.appManager == null) {
            this.appManager = new DefaultWorkspaceApplicationManager();
        }
        return appManager;
    }
    ...
    /**
     * @return the full name of the document that holds the XWiki.XWikiGroups XObjects with all the workspace members
     */
    ...
    /**
     * @return the full name of the document that holds the XWiki.XWikiGroups XObjects with all the workspace admins
     */
    ...
    protected void updateWorkspaceSpaceClass(XWikiContext context) throws XWikiException
    {
        ...
        needsUpdate |=
            bclass.addStaticListField(Workspace.WORKSPACE_ACCESSLEVEL, "Workspace Access Level", 1, false,
                levelsFields, "select");
        needsUpdate |=
            bclass.addStaticListField(Workspace.WORKSPACE_SPACETYPE, "Workspace Space Type", 1, false,
                Workspace.WORKSPACE_SPACETYPE_WORK + "|" + Workspace.WORKSPACE_SPACETYPE_USER + "|"
                    + Workspace.WORKSPACE_SPACETYPE_ORG, "select");
        needsUpdate |= bclass.addStaticListField(Workspace.WORKSPACE_COLOR, "Workspace Color", 1, false, colorField);
    ...
    /**
     * Operations executed after the actual {@link Space} creation. For a workspace, consist in intializing the space
     * rights, according to its visivility, private or public, and publish the first story of its activity stream .
     * {@inheritDoc}
     */
    public void postCreateSpace(String spaceName, XWikiContext context) throws SpaceManagerException
    {
        try {
            ...
            wasApi.addDocumentActivityEvent(w.getSpace(), new Document(doc, context), ActivityEventType.CREATE_SPACE,
     ...
     * {@link com.xpn.xwiki.plugin.spacemanager.api.SpaceManager#deleteSpace(String, boolean, XWikiContext)}, delete
     * inherited spaces documents (Applications spaces documents) of the workspace.
     * 
     * @param spaceName the name of the root space (web) of the workspace for which to delete application data. This is
     *            the name present in the 'parent' field of the XWikiPreferences object of the application spaces
     *            (webs).
    ...
    public void postDeleteSpace(String spaceName, boolean deleteData, XWikiContext context)
    {
        if (deleteData) {
            ...
            String hql =
                ", XWikiDocument as sp" + ", BaseObject as prefs, StringProperty as parent"
                    + " where sp.name='WebPreferences' and doc.web=sp.web"
                    + " and prefs.className='XWiki.XWikiPreferences' and prefs.name=sp.fullName"
                    + " and parent.id.id=prefs.id and parent.id.name='parent' and parent.value='" + spaceName + "'";
            try {
                List<XWikiDocument> allSpaceDocs = context.getWiki().getStore().searchDocuments(hql, context);
                for (XWikiDocument toBeDeleted : allSpaceDocs) {
                    // delete document, skipping the trash bin.
                    context.getWiki().deleteDocument(toBeDeleted, false, context);
                }
            } catch (XWikiException e) {
                // silently fail but log the error
                LOG.error("Failed to delete workspace data after deleting workspace [" + spaceName + "]", e);
            }
        }
    }
    ...
    private String getSpaceTypeForRequest(XWikiContext context)
    {
        ...
        if (context.getRequest().getParameter(getWorkspaceSpaceClassName() + "_0_spacetype") != null)
            spaceType = context.getRequest().getParameter(getWorkspaceSpaceClassName() + "_0_spacetype");
        else
            spaceType = Workspace.WORKSPACE_SPACETYPE_DEFAULT;
        if (spaceType.equals(Workspace.WORKSPACE_SPACETYPE_USER) || spaceType.equals(Workspace.WORKSPACE_SPACETYPE_ORG)
    ...
    private String getAccessLevelForRequest(XWikiContext context)
    {
        ...
        if (getSpaceTypeForRequest(context).equals(Workspace.WORKSPACE_SPACETYPE_USER)) {
            accessLevel = context.getWiki().getXWikiPreference("xws_userspace_defaultlevel", context);
            ...
            accessLevel = context.getRequest().getParameter(getWorkspaceSpaceClassName() + "_0_accesslevel");
            if (!isValidLevel(accessLevel))
    ...
    /**
     * Prepare a newly created Workspace by adding some document for it. Iterate over an application list, and for each
     * application to install in the workspace, copy or link inside the workspace resources defined by the application
     * object.
     * 
     * @throws SpaceManagerException
     */
    protected void prepareWorkspaceContent(String spaceName, XWikiContext context) throws SpaceManagerException
    {
        // set space WebPreferences content
        try {
            XWikiDocument pDoc = context.getWiki().getDocument(spaceName + ".WebPreferences", context);
        ...
        Set<String> available = getXWSApplicationManager().getAvailableApplicationsNames(spaceName, context);
        ...
        for (String application : requestedApps) {
            try {
                getXWSApplicationManager().installApplicationInSpace(application, spaceName, context);
     ...
     */
    public void makeSpacePrivate(String spaceName, XWikiContext context) throws WorkspacesManagerException
    {
        try {
    ...
    /**
     * Save the rights of a workspace to make it public : deny view right to XWiki.XWikiGuest but allow view right to
     * XWiki.XWikiAllGroup so that authenticated users can access the Workspace.
     * 
     * @throws WorkspacesManagerException
     */
    public void makeSpacePublic(String spaceName, XWikiContext context) throws WorkspacesManagerException
    {
        try {
            // Allow view to all group
            XWikiDocument doc = context.getWiki().getDocument(spaceName + ".WebPreferences", context);
            BaseObject ag = doc.getObject("XWiki.XWikiGlobalRights", "groups", "XWiki.XWikiAllGroup");
            if (ag == null) {
    ...
    /**
     * Save rights of a workspace to make it open : deny view to XWiki.XWikiGuest but allow both wiew and edit to
     * XWiki.XWikiAllGroup so that authenticated users can access in view et edit mode the workspace.
     * 
     * @throws WorkspacesManagerException
     */
    public void makeSpaceOpen(String spaceName, XWikiContext context) throws WorkspacesManagerException
    {
        try {
