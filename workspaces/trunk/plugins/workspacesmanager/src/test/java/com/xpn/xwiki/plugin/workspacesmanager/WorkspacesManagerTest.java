...
public class WorkspacesManagerTest extends AbstractWorkspacesTest
{
     ...
     */
    protected Mock mockXWSApplicationManager;
    
    /**
     * A Mock for workspaces activity stream.
     */
    protected Mock mockXWSActivityStream;
     ...
     * @throws Exception when an error occur during the test setup.
     */
    protected void setUp() throws Exception
    {
        ...
        this.xwsManager = new WorkspacesManager("xwsmgr", WorkspacesManager.class.getName(), context);

        this.xwsManager.setMailNotification(false);
        this.xwsManager.init(context);

        this.mockXWSApplicationManager = mock(WorkspaceApplicationManager.class);
        this.mockXWSApplicationManager.stubs().method("getAvailableApplicationsNames").will(
            returnValue(new HashSet<String>()));

        this.mockXWSActivityStream = mock(WorkspaceActivityStream.class);
        this.mockXWSActivityStream.stubs().method("addWorkspaceMemberActivityEvent");

        this.xwsManager.setWorkspacesActivityStream((WorkspaceActivityStream) this.mockXWSActivityStream.proxy());

        ((WorkspacesManagerExtension) xwsManager.getSpaceManagerExtension())
            .setApplicationManager((WorkspaceApplicationManager) mockXWSApplicationManager.proxy());
    }

    /**
     * Assert that a user cannot create himself multiple personnal spaces from http request.
     * 
     * @throws SpaceManagerException when an error occur during the test
     */
