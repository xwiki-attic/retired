...
public class ActiveHTMLPanel extends Composite implements HasHTML {
    ...
    /**
     * The content selection object providing access to the range of selected
     * words and to the real selected text
     * 
     * @author kotelnikov
     */
    public class WordSelection {
         ...
         * 
         */
        protected WordSelection() {
            Selection s = Selection.getSelection();
            Element start = s.getStartElement();
            Element end = s.getEndElement();
            String startId = getElementId(start);
            String endId = getElementId(end);
            if (isWordId(startId) && isWordId(endId)) {
                int from = getWordPosById(startId);
                int to = getWordPosById(endId);
        ...
        /**
         * Returns the real selected text. It can contain only parts of words
            ...
            switch (DOM.eventGetType(event)) {
                case Event.ONCLICK:
                    ...
                    if (fWordClickListeners != null) {
                        for (Iterator iterator = fWordClickListeners.iterator(); iterator
                            .hasNext();) {
                            IWordClickListener listener = (IWordClickListener) iterator
                                .next();
    ...
    public void addWordClickListener(IWordClickListener listener) {
        if (fWordClickListeners == null)
            fWordClickListeners = new ArrayList();
     ...
     */
    private int getWordPosById(String id) {
        if (!id.startsWith(fWidetIdPrefix))
            return -1;
     ...
     *         wrapper
     */
    private boolean isWordId(String id) {
    ...
    public void removeWordClickListener(IWordClickListener listener) {
        if (fWordClickListeners != null)
            fWordClickListeners.remove(listener);
    }
            ...
            if (e != null) {
                ...
                if (spaceStyle != null && i < len - 1) {
                    ...
                    while (e != null) {
                        ...
                        if (siblingId == null || !siblingId.startsWith(id))
                            break;
                        setStyleName(e, wordStyle);
                        e = DOM.getNextSibling(e);
                    }
