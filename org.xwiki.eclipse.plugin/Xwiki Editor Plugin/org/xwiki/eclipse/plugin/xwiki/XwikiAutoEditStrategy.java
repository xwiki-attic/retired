...
public class XwikiAutoEditStrategy extends DefaultIndentLineAutoEditStrategy {
	...
	public void customizeDocumentCommand(IDocument document, DocumentCommand command)
	{
			...
			try {
				...
				if(region.getLength()>2)
				{
					...
					if(arr.indexOf(c.toString()) > -1)
					{
						...
						if(c.toString().toCharArray()[0] == '*') //TODO better way to do this?
						{
							//starts with '* '
							int i=1;
		...
		//An enter button pressed is taken as intention to start a new paragraph, otherwise, it shall word wrap.
