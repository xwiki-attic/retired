...
public class LucenePlugin extends XWikiDefaultPlugin implements XWikiPluginInterface
{
    ...
    private static final String DEFAULT_SEARCH_TYPE = "xwiki.plugins.lucene.distributedSearch";

    private static final String DEFAULT_HOSTNAME = "xwiki.plugins.lucene.hostname";

    private static final String DEFAULT_PORT = "xwiki.plugins.lucene.port";

    private static final String DEFAULT_MASTER = "xwiki.plugins.lucene.master";

    private static final String DEFAULT_SLAVE_LIST = "xwiki.plugins.lucene.slavelist";
    
    private static final String GLOBAL_INDEX_DIR = "xwiki.pluginglglobalindexdir";
    
    private static final String SLAVES_INFO_PORT = "xwiki.plugins.lucene.slavesinfoport";
    ...
    private GlobalIndexUpdater globalIndexUpdater;
    ...
    private String searchType = "false";
   ...
   /**
    * API to search remote multiple indexes
    */
    //public RemoteIndexesSearcher remoteIndexesSearcher;
        ...
        ParseException
    {
        ...
        // Perform the actual search
	Hits hits;
	if(this.searchType.equals("false"))
	{
		// Normal search
        	hits = (sort == null) ? searcher.search(q) : searcher.search(q, sort);
	}
	else
	{
		//Distributed Search.
		//System.out.println("Distributed Search");
        	hits = globalIndexUpdater.getHits(q);
        ...
        throws ParseException
    {
	    ...
	    /** query to search in "title" field; If found boost the score of document by 4times*/
            QueryParser qp = new QueryParser("title", analyzer);
            parsedQuery = qp.parse(query);
	    parsedQuery.setBoost(4);
	    bQuery.add(parsedQuery,BooleanClause.Occur.SHOULD);
	    /** query to search in "name" field; If found boost the score of document by 2times*/
	    qp=null;
	    parsedQuery=null;
	    qp = new QueryParser("name", analyzer);
	    parsedQuery = qp.parse(query);
            parsedQuery.setBoost(2);
            bQuery.add(parsedQuery,BooleanClause.Occur.SHOULD);
	    /** Finally search for query in "ft" (fulltext) field; No need of boost factor here*/
	    qp=null;
            parsedQuery=null;
            qp = new QueryParser("ft", analyzer);
            parsedQuery = qp.parse(query);
            parsedQuery.setBoost(1);
            bQuery.add(parsedQuery,BooleanClause.Occur.MUST);
        }
	...
	this.searchType = config.getProperty(DEFAULT_SEARCH_TYPE,"false");

	if(this.searchType.equals("true"))
	{
		String hostname = config.getProperty(DEFAULT_HOSTNAME, "127.0.0.1");
		String port = config.getProperty(DEFAULT_PORT,"1099");
		String master = config.getProperty(DEFAULT_MASTER,hostname);
		String slavelist = config.getProperty(DEFAULT_SLAVE_LIST,hostname+":"+port);
		String slaveinfoport = config.getProperty(SLAVES_INFO_PORT,"1100");
		IndexSearchServer searchServer = new IndexSearchServer(indexDirs,hostname,port,analyzer);
		searchServer.setServiceName("XWiki_LuceneMultiSearch");
		searchServer.start();
        	LOG.info("lucene search server initialized.");
		try{
		Thread.sleep(1000);
		}catch(Exception e){}

		//remoteIndexesSearcher = new RemoteIndexesSearcher(slavelist, "XWiki_RemoteMultiIndexesSearcher");
		indexUpdater.setHostname(hostname);
		indexUpdater.setSearchServer(searchServer);
		indexUpdater.setMasterInfo(master+":"+slaveinfoport);
		if(hostname.equals(master))
		{
			//The current installation is on master node
			//
			this.globalIndexDir = config.getProperty(GLOBAL_INDEX_DIR);
		        if (globalIndexDir == null || indexDirs.equals("")) {
            			File workDir = context.getWiki().getWorkSubdirectory("globalIndexLucene", context);
            			globalIndexDir = workDir.getAbsolutePath();
       			 }

            		File slavesInfoLucene = context.getWiki().getWorkSubdirectory("slavesinformation", context);
			SlavesInfo slavesInfo = new SlavesInfo(slavesInfoLucene.getAbsolutePath(),
							master+":"+slaveinfoport,
							slavelist);
			//slavesInfo.init();
	
			globalIndexUpdater = new GlobalIndexUpdater(globalIndexDir,analyzer,slavesInfo);
			globalIndexUpdater.init();
			globalIndexUpdater.start();
		
		}
	}
	else
	{
		indexUpdater.setSearchServer(null);
		indexUpdater.setMasterInfo(null);
	}
