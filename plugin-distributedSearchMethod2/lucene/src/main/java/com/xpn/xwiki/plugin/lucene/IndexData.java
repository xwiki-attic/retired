...
public abstract class IndexData
{
    ...
    //next two fields are required to accompish distributed search task.
    ...
    private static final String DEFAULT_SERVLET_PORT = "xwiki.plugins.lucene.servletPort";
	...
	// Indexing the TIMESTAMP at which document has been indexed.

	DecimalFormat df = new DecimalFormat("000000000000000000000");
	Date dt = new Date();
        luceneDoc.add(new Field(IndexFields.TIMESTAMP,
        ...
        if (hostname != null) {
            luceneDoc.add(new Field(IndexFields.HOSTNAME,
                hostname,
                Field.Store.YES,
                Field.Index.TOKENIZED));
        }
        if (servletPort != null) {
            luceneDoc.add(new Field(IndexFields.SERVLET_PORT,
                servletPort,
                Field.Store.YES,
                Field.Index.TOKENIZED));
        }
    ...
    public void setHostname(Properties config)
    {
    ...
    public void setServletPort(Properties config)
    {
	String servletPort=config.getProperty(DEFAULT_SERVLET_PORT, "8080");
