...
public class XWikiEclipsePageSummaryAdapter extends WorkbenchAdapter {
	...
	public Object[] getChildren(Object object) {
		if (object instanceof XWikiEclipsePageSummary) {
			...
			try {
				List<XWikiEclipseObjectSummary> result = pageSummary
						.getDataManager().getObjects(
	...
	public ImageDescriptor getImageDescriptor(Object object) {
		if (object instanceof XWikiEclipsePageSummary) {
			XWikiEclipsePageSummary pageSummary = (XWikiEclipsePageSummary) object;
			if (pageSummary.getDataManager().isInConflict(
					pageSummary.getData().getId()))
				return UIPlugin
						.getImageDescriptor(UIConstants.PAGE_CONFLICT_ICON);

			if (pageSummary.getDataManager().isLocallyAvailable(pageSummary))
				return UIPlugin
						.getImageDescriptor(UIConstants.PAGE_LOCALLY_AVAILABLE_ICON);
			else
