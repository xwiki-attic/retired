...
public class PagePreview extends ViewPart implements ISelectionListener,
		ICoreEventListener {
	...
	private class ShowAddressBarAction extends Action {
		...
		public void run() {
			if (isChecked()) {
				...
				GridData gridData = (GridData) addressBarComposite
						.getLayoutData();
	...
	public void init(IViewSite site) throws PartInitException {
		...
		NotificationManager.getDefault().addListener(
				...
				new CoreEvent.Type[] { CoreEvent.Type.DATA_MANAGER_CONNECTED,
						CoreEvent.Type.DATA_MANAGER_DISCONNECTED,
						CoreEvent.Type.DATA_MANAGER_REGISTERED,
						CoreEvent.Type.DATA_MANAGER_UNREGISTERED,
						CoreEvent.Type.OBJECT_REMOVED,
						CoreEvent.Type.OBJECT_STORED,
						CoreEvent.Type.PAGE_REMOVED,
						CoreEvent.Type.PAGE_STORED,
						CoreEvent.Type.PAGE_SELECTED,
	...
	public void createPartControl(Composite parent) {
		...
		GridLayoutFactory.fillDefaults().margins(0, 0).spacing(0, 0).applyTo(
				mainComposite);

		addressBarComposite = new Composite(mainComposite, SWT.NONE);
		GridLayoutFactory.fillDefaults().numColumns(3).margins(0, 5).spacing(0,
				0).applyTo(addressBarComposite);
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true,
				false).exclude(true).applyTo(addressBarComposite);
		...
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true,
				false).applyTo(urlText);
		addressBarComposite.setBackground(Display.getCurrent().getSystemColor(
				SWT.COLOR_WHITE));
		addressBarComposite.setVisible(false);
		...
		GridLayoutFactory.fillDefaults().margins(0, 0).applyTo(
				notConnectedComposite);
		notConnectedComposite.setBackground(parent.getDisplay().getSystemColor(
				SWT.COLOR_WHITE));
		...
		noPageSelectedComposite.setBackground(parent.getDisplay()
				.getSystemColor(SWT.COLOR_WHITE));
		GridLayoutFactory.fillDefaults().margins(0, 0).applyTo(
	...
	public void selectionChanged(IWorkbenchPart part, ISelection selection) {
		Object selectedObject = UIUtils
				.getFirstSelectedObjectsFromSelection(selection);
		if (selectedObject instanceof XWikiEclipsePageSummary) {
			...
			update(pageSummary.getData().getUrl(), pageSummary.getDataManager()
					.isConnected());
		}
		...
		if (selectedObject instanceof XWikiEclipseObjectSummary) {
			...
			update(objectSummary.getPageSummary() != null ? objectSummary
					.getPageSummary().getUrl() : null, objectSummary
	...
	public void handleCoreEvent(CoreEvent event) {
		...
		if (object instanceof XWikiEclipseObject) {
			...
			update(xwikiObject.getPageSummary() != null ? xwikiObject
					.getPageSummary().getUrl() : null, xwikiObject
					.getDataManager().isConnected());
		}
	}
