...
public class ListPropertyEditor extends BasePropertyEditor {
	...
	public Composite createControl(Composite parent) {
		...
		String unModifiableAttributeString = property
				.getAttribute("unmodifiable");
		if ((unModifiableAttributeString == null)
				|| unModifiableAttributeString.equals("0"))
	...
	@SuppressWarnings("unchecked")
	private Control createReadOnlyStaticListField(FormToolkit toolkit,
			Composite parent, final XWikiEclipseObjectProperty property) {
		...
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true,
				false).applyTo(currentSelectionLabel);

		String multiSelectAttributeString = property
				.getAttribute("multiSelect").toString();
		if ((multiSelectAttributeString == null)
				|| multiSelectAttributeString.equals("0"))
			multiSelect = false;
		else
			multiSelect = true;

		listViewer = new ListViewer(composite, SWT.BORDER | SWT.V_SCROLL
				| SWT.FULL_SELECTION | (multiSelect ? SWT.MULTI : SWT.NONE));
		...
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).span(2, 1)
				.hint(0, 5 * listViewer.getList().getItemHeight()).grab(true,
						false).applyTo(listViewer.getList());

		return composite;
	}

	private Control createModifiableStaticListField(FormToolkit toolkit,
			final Composite parent, final XWikiEclipseObjectProperty property) {
		Composite composite = toolkit.createComposite(parent);
		GridLayoutFactory.fillDefaults().numColumns(2).applyTo(composite);
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.FILL).grab(true,
				true).applyTo(composite);
		...
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).hint(0,
				5 * listViewer.getList().getItemHeight()).grab(true, false)
		...
		button.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				InputDialog input = new InputDialog(parent.getShell(),
						"Add value",
						"Please enter the value to add to the list", "", null);
				input.open();
