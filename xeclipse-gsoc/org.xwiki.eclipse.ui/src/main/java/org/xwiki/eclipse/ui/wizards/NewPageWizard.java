...
public class NewPageWizard extends Wizard implements INewWizard {
		...
		try {
			getContainer().run(true, false, new IRunnableWithProgress() {
				public void run(IProgressMonitor monitor)
					...
					try {
						monitor.beginTask("Creating page...",
								IProgressMonitor.UNKNOWN);
						final XWikiEclipsePage page = dataManager.createPage(
								newPageWizardState.getSpace(),
								newPageWizardState.getName(),
								newPageWizardState.getTitle(),
								"Write here content");
			...
			WizardPage currentPage = (WizardPage) getContainer()
					.getCurrentPage();
	...
	public boolean canFinish() {
		...
		if ((newPageWizardState.getSpace() == null)
				|| (newPageWizardState.getSpace().length() == 0))
			return false;

		if ((newPageWizardState.getName() == null)
				|| (newPageWizardState.getName().length() == 0))
			return false;

		if ((newPageWizardState.getTitle() == null)
				|| (newPageWizardState.getTitle().length() == 0))
			return false;
