	...
	public void customizeDocumentCommand(IDocument document,
		...
		if (command.text.equals(">"))
			try {
				 ...
				 * document.getChar(command.offset-3)=='<') { int line =
				 * document.getLineOfOffset(command.offset); String
				 * indent=getIndent(document,line); command.text =
				 * ">"+"\n\t"+indent+"\n"+indent+"</dl>";
		...
		/*
		 * if(command.text.equals("\n")) { try { int line =
		 * document.getLineOfOffset(command.offset); IRegion region =
		 * document.getLineInformation(line); int offset = region.getOffset();
		 * if(region.getLength()>2) { //System.out.println("Region: "+offset+"
		 * "+region.getLength()); Character c =new
		 * Character(document.getChar(offset)); String arr = "aAiIghk";
		 * if(arr.indexOf(c.toString()) > -1) { //character starts with one of
		 * them. if(document.getChar(offset+1)=='.' &&
		 * document.getChar(offset+2)==' ') { command.text="\n"+c+". ";
		 * configureCommand(command,4); } } } } catch (BadLocationException e) {
