...
public class PageConflictDialog extends TitleAreaDialog {
	...
	static class CompareElement implements ITypedElement,
		...
		public InputStream getContents() throws CoreException {
			return new ByteArrayInputStream(page.getData().getContent()
					.getBytes());
		}
	...
	protected Control createContents(Composite parent) {
		...
		setTitleImage(UIPlugin.getImageDescriptor(UIConstants.CONFLICT_BANNER)
				.createImage());
	...
	protected Control createDialogArea(Composite parent) {
		...
		if (ancestorPage != null)
			cc.setAncestorLabel(String.format("%s (Original)", ancestorPage
					.getData().getId()));

		cc.setLeftLabel(String.format("%s (Local)", page.getData().getId()));
		cc.setRightLabel(String.format("%s (Conflicting)", conflictingPage
				.getData().getId()));
		...
		if (ancestorPage != null) {
			...
			viewer.setInput(new DiffNode(null, Differencer.CONFLICTING,
					ancestor, left, right));
		} else
