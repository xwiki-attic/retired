...
public class TextAreaPropertyEditor extends BasePropertyEditor {
	...
	public Composite createControl(final Composite parent) {
		...
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true,
				false).hint(0, textArea.getLineHeight() * 5).applyTo(textArea);
		...
		Button button = toolkit.createButton(composite,
				"Open editor in a separate window...", SWT.PUSH);
		...
		button.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				TextAreaPropertyEditorDialog dialog = new TextAreaPropertyEditorDialog(
						parent.getShell(), property);
				int result = dialog.open();
				if (result == Window.OK) {
					textArea.setText(dialog.getText() != null ? dialog
							.getText() : "");
					firePropertyModifyListener();
				}
			}
