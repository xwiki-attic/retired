...
public class RenamePageDialog extends TitleAreaDialog {
	...
	public RenamePageDialog(Shell parentShell,
	...
	protected void createButtonsForButtonBar(Composite parent) {
		Button button = createButton(parent, IDialogConstants.OK_ID, "Rename",
				true);
	...
	protected Control createDialogArea(Composite parent) {
		...
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true,
				false).applyTo(comboViewer.getControl());
		comboViewer.setContentProvider(new IStructuredContentProvider() {
			public Object[] getElements(Object inputElement) {
				...
				if (runnable.getResult() != null) {
					...
					int i = 0;
					for (XWikiEclipseSpaceSummary spaceSummary : runnable
							.getResult()) {
		...
		comboViewer.setSelection(new StructuredSelection(pageSummary.getData()
				.getSpace()));
