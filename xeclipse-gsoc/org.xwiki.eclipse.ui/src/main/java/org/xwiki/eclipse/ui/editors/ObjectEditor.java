...
public class ObjectEditor extends EditorPart {
	...
	public void doSave(IProgressMonitor monitor) {
		...
		SafeRunner.run(new XWikiEclipseSafeRunnable() {
			public void run() throws Exception {
				input.getObject().getDataManager().storeObject(
						input.getObject());
			}
		});
	...
	public void createPartControl(Composite parent) {
		...
		for (XWikiEclipseObjectProperty property : object.getProperties()) {
			...
			if ("com.xpn.xwiki.objects.classes.StringClass".equals(property
				...
				propertyEditor = new StringPropertyEditor(toolkit, scrolledForm
						.getBody(), property);

			if ("com.xpn.xwiki.objects.classes.PasswordClass".equals(property
					.getAttribute(XWikiClass.XWIKICLASS_ATTRIBUTE)))
				propertyEditor = new PasswordPropertyEditor(toolkit,
						scrolledForm.getBody(), property);

			if ("com.xpn.xwiki.objects.classes.NumberClass".equals(property
					.getAttribute(XWikiClass.XWIKICLASS_ATTRIBUTE)))
				propertyEditor = new NumberPropertyEditor(toolkit, scrolledForm
						.getBody(), property);

			if ("com.xpn.xwiki.objects.classes.BooleanClass".equals(property
					.getAttribute(XWikiClass.XWIKICLASS_ATTRIBUTE)))
				propertyEditor = new BooleanPropertyEditor(toolkit,
						scrolledForm.getBody(), property);

			if ("com.xpn.xwiki.objects.classes.TextAreaClass".equals(property
					.getAttribute(XWikiClass.XWIKICLASS_ATTRIBUTE)))
				propertyEditor = new TextAreaPropertyEditor(toolkit,
						scrolledForm.getBody(), property);

			if ("com.xpn.xwiki.objects.classes.DateClass".equals(property
					.getAttribute(XWikiClass.XWIKICLASS_ATTRIBUTE)))
				propertyEditor = new DatePropertyEditor(toolkit, scrolledForm
						.getBody(), property);
			...
			if ("com.xpn.xwiki.objects.classes.StaticListClass".equals(property
					...
					|| "com.xpn.xwiki.objects.classes.DBListClass"
							.equals(property
									.getAttribute(XWikiClass.XWIKICLASS_ATTRIBUTE)))
				propertyEditor = new ListPropertyEditor(toolkit, scrolledForm
						.getBody(), property);
			...
			if (propertyEditor != null) {
				...
				propertyEditor
						.addPropertyModifyListener(new IPropertyModifyListener() {
	...
	public void setFocus() {
		...
		NotificationManager.getDefault().fireCoreEvent(
				CoreEvent.Type.OBJECT_SELECTED, this, object);
	}
	...
	private void updateInfo() {
		if (scrolledForm != null) {
			...
			if (input != null) {
				XWikiEclipseObject object = input.getObject();
				scrolledForm.setText(String.format("%s (Class %s) on page %s",
						object.getName(), object.getXWikiClass().getId(),
						object.getData().getPageId()));

				scrolledForm.getForm().setMessage(
						object.getDataManager().getName());
			}
		}
	}
...
class ObjectEditorContentOutlinePage extends ContentOutlinePage {
	...
	public ObjectEditorContentOutlinePage(XWikiEclipseObject object,
			ObjectEditor editor) {
		this.object = object;
		this.editor = editor;
	}
		...
		treeViewer.addSelectionChangedListener(new ISelectionChangedListener() {
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (!selection.isEmpty())
					editor.scrollTo((XWikiEclipseObjectProperty) selection
							.getFirstElement());
			}
