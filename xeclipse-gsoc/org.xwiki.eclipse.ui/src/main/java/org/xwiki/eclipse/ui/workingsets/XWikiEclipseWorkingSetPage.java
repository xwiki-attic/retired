...
public class XWikiEclipseWorkingSetPage extends WizardPage implements
		IWorkingSetPage {
	...
	class WorkingSetCheckboxTreeViewer extends CheckboxTreeViewer {
		...
		public WorkingSetCheckboxTreeViewer(Composite parent, int style,
		...
		private void setChecked(TreeItem item, Object o) {
			if (o instanceof DataManager) {
				...
				item.setChecked(UIUtils.isXWikiEcipseIdInWorkingSet(dataManager
						.getXWikiEclipseId(), workingSet));
			}

			if (o instanceof ModelObject) {
				ModelObject modelObject = (ModelObject) o;
				item.setChecked(UIUtils.isXWikiEcipseIdInWorkingSet(modelObject
						.getXWikiEclipseId(), workingSet));
			}
		}
	...
	public void finish() {
		...
		for (Object object : treeViewer.getCheckedElements()) {
			if (object instanceof DataManager)
				elements.add(new XWikiEclipseElementId(((DataManager) object)
						.getXWikiEclipseId()));

			if (object instanceof ModelObject)
				elements.add(new XWikiEclipseElementId(((ModelObject) object)
						.getXWikiEclipseId()));
		}
		...
		if (workingSet == null) {
			...
			workingSet = workingSetManager.createWorkingSet(workingSetName
					.getText(), elements.toArray(new IAdaptable[0]));
		} else
	...
	public void createControl(Composite parent) {
		...
		treeViewer = new WorkingSetCheckboxTreeViewer(composite, SWT.BORDER,
				workingSet);
