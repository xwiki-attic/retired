...
public class OpenPageDialog extends FilteredItemsSelectionDialog {
	...
	private static class OpenPageLabelProvider extends LabelProvider {
		...
		public String getText(Object element) {
			if (element instanceof XWikiEclipsePageSummary) {
				...
				return String.format("%s (%s)", xwikiPage.getData().getId(),
						xwikiPage.getDataManager().getName());
			}
	...
	public OpenPageDialog(Shell shell, DataManager dataManager) {
		...
		OpenPageLabelProvider labelProvider = new OpenPageLabelProvider(
		...
		if (dataManager != null)
			...
			for (DataManager dm : DataManagerRegistry.getDefault()
					.getDataManagers())
	...
	protected Control createExtendedContentArea(Composite parent) {
		...
		label
				.setText("Select the connections to be searched for opening pages:");

		CheckboxTreeViewer dataManagerTreeViewers = new CheckboxTreeViewer(
				composite, SWT.BORDER);
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true,
				false).applyTo(dataManagerTreeViewers.getControl());
		dataManagerTreeViewers.setContentProvider(new ITreeContentProvider() {
			...
			public Object[] getElements(Object inputElement) {
				return DataManagerRegistry.getDefault().getDataManagers()
						.toArray();
			}
	...
	protected void fillContentProvider(AbstractContentProvider contentProvider,
			ItemsFilter itemsFilter, final IProgressMonitor progressMonitor)
		...
		for (DataManager dataManager : targetDataManagers) {
			...
			/*
			 * If we don't already have summaries, then fetch them from the data
			 * manager
			 */
			if (dataManagerToPageSummariesMap.get(currentDataManager) == null)
				SafeRunner.run(new XWikiEclipseSafeRunnable() {
					public void run() throws Exception {
						...
						List<XWikiEclipseSpaceSummary> spaces = currentDataManager
								.getSpaces();
						for (XWikiEclipseSpaceSummary space : spaces) {
							if (progressMonitor.isCanceled())
								return;

							List<XWikiEclipsePageSummary> pages = currentDataManager
									.getPages(space.getData().getKey());
							for (XWikiEclipsePageSummary pageSummary : pages)
								pageSummaries.add(pageSummary);
						}

						dataManagerToPageSummariesMap.put(currentDataManager,
								pageSummaries);
					}
				});
			...
			for (XWikiEclipsePageSummary pageSummary : dataManagerToPageSummariesMap
					.get(currentDataManager))
	...
	protected Comparator getItemsComparator() {
		return new Comparator() {
			public int compare(Object o1, Object o2) {
				...
				return page1.getData().getTitle().compareTo(
						page2.getData().getTitle());
			}
		};
	}
}
