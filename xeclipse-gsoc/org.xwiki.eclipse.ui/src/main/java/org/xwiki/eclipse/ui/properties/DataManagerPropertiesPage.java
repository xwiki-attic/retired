	...
	protected Control createContents(Composite parent) {
		...
		SafeRunner.run(new XWikiEclipseSafeRunnable() {
			public void run() throws Exception {
				dataManager = (DataManager) getElement().getAdapter(
						DataManager.class);
				...
				if (dataManager.isConnected()) {
					...
					for (Functionality functionality : dataManager
							.getSupportedFunctionalities())
						f.format("%s ", functionality);
					label.setText(String.format("Connected. Support for: %s", f
							.toString()));
				} else
				...
				GridDataFactory.fillDefaults().align(SWT.FILL, SWT.FILL).grab(
						true, false).applyTo(endpointText);
				String endpoint = dataManager.getEndpoint();
				endpointText.setText(endpoint != null ? endpoint
						: "No endpoint defined");
				...
				GridDataFactory.fillDefaults().align(SWT.FILL, SWT.FILL).grab(
						true, false).applyTo(userNameText);
				...
				GridDataFactory.fillDefaults().align(SWT.FILL, SWT.FILL).grab(
						true, false).applyTo(passwordText);
				...
				GridDataFactory.fillDefaults().align(SWT.FILL, SWT.FILL).grab(
						true, false).applyTo(autoConnect);
