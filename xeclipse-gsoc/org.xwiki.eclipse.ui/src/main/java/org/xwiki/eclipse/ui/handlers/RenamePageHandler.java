	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (selectedObjects.size() == 1) {
			...
			if (selectedObject instanceof XWikiEclipsePageSummary) {
				...
				if (!pageSummary.getDataManager().getSupportedFunctionalities()
						.contains(Functionality.RENAME)) {
					UIUtils
							.showMessageDialog(
				...
				final RenamePageDialog dialog = new RenamePageDialog(
						HandlerUtil.getActiveShell(event), pageSummary);
				dialog.open();

				if (dialog.getReturnCode() == IDialogConstants.OK_ID)
					SafeRunner.run(new XWikiEclipseSafeRunnable() {
						public void run() throws Exception {
							pageSummary.getDataManager().renamePage(
