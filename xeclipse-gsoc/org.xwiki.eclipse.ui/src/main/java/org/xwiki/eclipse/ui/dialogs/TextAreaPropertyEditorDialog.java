...
public class TextAreaPropertyEditorDialog extends Dialog {
	...
	public TextAreaPropertyEditorDialog(Shell parentShell,
			XWikiEclipseObjectProperty object) {
		super(parentShell);
		property = object;
	}
	...
	protected Control createContents(Composite parent) {
		...
		form.setText(String.format("Property %s of object %s on page %s",
				property.getPrettyName(), property.getObject().getName(),
				property.getObject().getData().getPageId()));
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.FILL).grab(true,
				true).applyTo(form);
		GridLayoutFactory.fillDefaults().applyTo(form.getBody());

		final StyledText styledText = new StyledText(form.getBody(), SWT.BORDER);
		styledText.setText(property.getValue() != null ? (String) property
				.getValue() : "");
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.FILL).grab(true,
				true).applyTo(styledText);
		...
		GridLayoutFactory.fillDefaults().numColumns(3).extendedMargins(0, 0, 0,
				10).applyTo(buttonBar);
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.FILL).grab(true,
				false).applyTo(buttonBar);

		Label filler = toolkit.createLabel(buttonBar, " ");
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true,
				false).applyTo(filler);
