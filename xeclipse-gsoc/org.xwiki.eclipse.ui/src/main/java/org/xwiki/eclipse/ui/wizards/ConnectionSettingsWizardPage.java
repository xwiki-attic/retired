...
public class ConnectionSettingsWizardPage extends WizardPage {
	...
	public void createControl(Composite parent) {
		newConnectionWizardState = ((NewConnectionWizard) getWizard())
				.getNewConnectionWizardState();
		...
		/*
		 * Connection name (this will be the corresponding project name in the
		 * workspace)
		 */
		connectionNameText = new Text(composite, SWT.BORDER);
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.FILL).grab(true,
				false).applyTo(connectionNameText);
		connectionNameText.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				newConnectionWizardState.setConnectionName(connectionNameText
						.getText());
	...
	public boolean isPageComplete() {
		if ((connectionNameText.getText() == null)
				|| (connectionNameText.getText().length() == 0)) {
			setErrorMessage("Connection name must be specified.");
			return false;
		}

		if ((serverUrlText.getText() == null)
				|| !(serverUrlText.getText().startsWith("http://") || serverUrlText
