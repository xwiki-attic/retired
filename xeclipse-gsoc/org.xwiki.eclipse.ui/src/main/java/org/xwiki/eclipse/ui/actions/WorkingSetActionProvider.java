...
public class WorkingSetActionProvider extends CommonActionProvider {
	...
	public void fillActionBars(IActionBars actionBars) {
		if (!contributedToViewMenu) {
			actions = new WorkingSetFilterActionGroup(getActionSite()
					.getViewSite().getShell(), new IPropertyChangeListener() {
				public void propertyChange(PropertyChangeEvent event) {
					if (event.getNewValue() != null)
						getActionSite().getStructuredViewer().setInput(
								event.getNewValue());
					else
						/*
						 * Actually it doesn't really matter what input we
						 * specify here
						 */
						getActionSite().getStructuredViewer().setInput(
