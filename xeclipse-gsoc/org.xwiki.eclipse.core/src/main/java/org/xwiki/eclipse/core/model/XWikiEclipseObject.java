...
public class XWikiEclipseObject extends ModelObject {
	 ...
	 * 
	 * @param dataManager
	 *            The data manager that returned this object.
	 * @param data
	 *            The actual XWiki object description.
	 * @param xwikiClass
	 *            The XWiki class for this object.
	 */
	public XWikiEclipseObject(DataManager dataManager, XWikiObject data,
			XWikiClass xwikiClass, XWikiPageSummary pageSummary) {
		super(dataManager);
	...
	public String getName() {
		...
		if (name == null)
			if (data.getId() == -1)
				...
				name = String.format("%s[%d]", data.getClassName(), data
						.getId());
