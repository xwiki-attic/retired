...
/**
 * This class implements a local data storage for XWiki elements that uses the
 * Eclipse resource component. The local storage is rooted at an IFolder passed
 * to the constructor. The structure of the local storage is the following:
 * 
 * <pre>
 * Root 
 * + index
 ...
 * </pre>
 * 
 * All xe* files contains an XML serialization of the corresponding XWiki
 * Eclipse elements.
 * 
 * @author fmancinelli
 */
public class LocalXWikiDataStorage implements IDataStorage {
	...
	public XWikiPage getPage(String pageId) throws XWikiEclipseException {
		try {
			IFolder pageFolder = CoreUtils.createFolder(baseFolder
					.getFolder(PAGES_DIRECTORY));

			IFile pageFile = pageFolder.getFile(getFileNameForPage(pageId));
			if (pageFile.exists()) {
				Map<String, Object> map = (Map<String, Object>) CoreUtils
						.readDataFromXML(pageFile);
	...
	@SuppressWarnings("unchecked")
	public List<XWikiPageSummary> getPages(String spaceKey)
			throws XWikiEclipseException {
		final List<XWikiPageSummary> result = new ArrayList<XWikiPageSummary>();

		IFolder spaceFolder = baseFolder.getFolder(INDEX_DIRECTORY).getFolder(
				spaceKey);
		if (spaceFolder.exists())
			try {
				List<IResource> spaceFolderResources = getChildResources(
						spaceFolder, IResource.DEPTH_ONE);
				for (IResource spaceFolderResource : spaceFolderResources)
					if (spaceFolderResource instanceof IFolder) {
						...
						List<IResource> pageFolderResources = getChildResources(
								pageFolder, IResource.DEPTH_ONE);
						...
						/*
						 * This can happen, for example, if the user stores an
						 * object and has never stored the page it belongs to:
						 * we have a folder named after the page id containing
						 * the object summary, but no page summary is available
						 * in that folder. In this case we build a reduced
						 * summary with the information extracted from the
						 * folder name.
						 */
						if (!pageSummaryFound) {
							String[] pageIdComponents = pageFolder.getName()
									.split("\\."); //$NON-NLS-1$
	...
	public List<SpaceSummary> getSpaces() throws XWikiEclipseException {
		...
		try {
			final IFolder indexFolder = CoreUtils.createFolder(baseFolder
					.getFolder(INDEX_DIRECTORY));

			List<IResource> indexFolderResources = getChildResources(
					indexFolder, IResource.DEPTH_ONE);
	...
	public XWikiPage storePage(final XWikiPage page)
			throws XWikiEclipseException {
		try {
			ResourcesPlugin.getWorkspace().run(new IWorkspaceRunnable() {
				...
				public void run(IProgressMonitor monitor) throws CoreException {
					...
					pageSummary
							.setTranslations(page.getTranslations() != null ? page
									.getTranslations()
									: new ArrayList());
					pageSummary.setUrl(page.getUrl());
					CoreUtils.writeDataToXML(baseFolder.getFolder(
							INDEX_DIRECTORY).getFolder(page.getSpace())
							.getFolder(page.getId()).getFile(
									getFileNameForPageSummary(pageSummary
											.getId())), pageSummary.toRawMap());

					/* Write the page */
					CoreUtils.writeDataToXML(baseFolder.getFolder(
							PAGES_DIRECTORY).getFile(
							getFileNameForPage(page.getId())), page.toRawMap());
				}
			}, null);
		} catch (CoreException e) {
			throw new XWikiEclipseException(e);
			...
			ResourcesPlugin.getWorkspace().run(new IWorkspaceRunnable() {
				public void run(IProgressMonitor monitor) throws CoreException {
					try {
						...
						/*
						 * Remove the index page folder with all the information
						 * (page and object summaries)
						 */
						IFolder indexPageFolder = baseFolder.getFolder(
								INDEX_DIRECTORY).getFolder(page.getSpace())
								.getFolder(pageId);
						...
						IFolder pageFolder = CoreUtils.createFolder(baseFolder
								.getFolder(PAGES_DIRECTORY));
						IFile pageFile = pageFolder
								.getFile(getFileNameForPage(pageId));
						if (pageFile.exists())
							pageFile.delete(true, null);
					} catch (XWikiEclipseException e) {
						throw new CoreException(new Status(IStatus.ERROR,
								CorePlugin.PLUGIN_ID, "Error", e));
					}
				}
			}, null);
	...
	public List<XWikiObjectSummary> getObjects(String pageId)
		...
		IFolder pageFolder = baseFolder.getFolder(INDEX_DIRECTORY).getFolder(
				page.getSpace()).getFolder(pageId);
	...
	public XWikiObject getObject(String pageId, String className, int objectId)
			throws XWikiEclipseException {
		try {
			IFolder objectsFolder = CoreUtils.createFolder(baseFolder
					.getFolder(OBJECTS_DIRECTORY));

			IFile objectFile = objectsFolder.getFile(getFileNameForObject(
					pageId, className, objectId));
			if (objectFile.exists()) {
				Map<String, Object> map = (Map<String, Object>) CoreUtils
						.readDataFromXML(objectFile);
	...
	public XWikiClass getClass(String classId) throws XWikiEclipseException {
		try {
			IFolder classesFolder = CoreUtils.createFolder(baseFolder
					.getFolder(CLASSES_DIRECTORY));

			IFile classFile = classesFolder
					.getFile(getFileNameForClass(classId));
			if (classFile.exists()) {
				Map<String, Object> map = (Map<String, Object>) CoreUtils
						.readDataFromXML(classFile);
	...
	public XWikiObject storeObject(final XWikiObject object)
			throws XWikiEclipseException {
		try {
			ResourcesPlugin.getWorkspace().run(new IWorkspaceRunnable() {
				public void run(IProgressMonitor monitor) throws CoreException {
					try {
						...
						CoreUtils.writeDataToXML(baseFolder.getFolder(
								...
								.getFolder(object.getPageId()).getFile(
										getFileNameForObjectSummary(
												objectSummary.getPageId(),
												objectSummary.getClassName(),
												objectSummary.getId())),
								objectSummary.toRawMap());

						/* Write the object */
						CoreUtils.writeDataToXML(baseFolder.getFolder(
								OBJECTS_DIRECTORY).getFile(
								getFileNameForObject(object.getPageId(), object
	...
	public void storeClass(final XWikiClass xwikiClass)
			throws XWikiEclipseException {
		try {
			ResourcesPlugin.getWorkspace().run(new IWorkspaceRunnable() {
				public void run(IProgressMonitor monitor) throws CoreException {
					CoreUtils.writeDataToXML(baseFolder.getFolder(
							CLASSES_DIRECTORY).getFile(
							getFileNameForClass(xwikiClass.getId())),
							xwikiClass.toRawMap());
				}
			}, null);
		} catch (CoreException e) {
	...
	public boolean exists(String pageId) {
		IFile pageFile = baseFolder.getFolder(PAGES_DIRECTORY).getFile(
				getFileNameForPage(pageId));
		return pageFile.exists();
	}

	public boolean exists(String pageId, String className, int objectId) {
		IFile objectFile = baseFolder.getFolder(OBJECTS_DIRECTORY).getFile(
	...
	private String getFileNameForObjectSummary(String pageId, String className,
			int id) {
		return String
				.format(
						"%s.%s.%d.%s", pageId, className, id, OBJECT_SUMMARY_FILE_EXTENSION); //$NON-NLS-1$
	}

	private String getFileNameForObject(String pageId, String className, int id) {
		return String.format(
				"%s.%s.%d.%s", pageId, className, id, OBJECT_FILE_EXTENSION); //$NON-NLS-1$
	}
	...
	public List<XWikiClassSummary> getClasses() throws XWikiEclipseException {
		...
		try {
			List<IResource> classFolderResources = getChildResources(baseFolder
					.getFolder(CLASSES_DIRECTORY), IResource.DEPTH_ONE);
	...
	public boolean removeObject(final String pageId, final String className,
			final int objectId) throws XWikiEclipseException {
		try {
			ResourcesPlugin.getWorkspace().run(new IWorkspaceRunnable() {
				public void run(IProgressMonitor monitor) throws CoreException {
					try {
						IFile file = baseFolder.getFolder(OBJECTS_DIRECTORY)
								.getFile(
										getFileNameForObject(pageId, className,
												objectId));
						if (file.exists())
						...
						file = baseFolder.getFolder(INDEX_DIRECTORY).getFolder(
								page.getSpace()).getFolder(pageId).getFile(
								getFileNameForObjectSummary(pageId, className,
										objectId));
						if (file.exists())
							file.delete(true, null);
					} catch (XWikiEclipseException e) {
